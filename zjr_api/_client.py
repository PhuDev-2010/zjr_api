...
class ZaloAPI(object):
	...
	def _encode(self, params):
		return _util.zalo_encode(
			params, 
			self._state._config.get("secret_key")
		)
		
	def _decode(self, params):
		return _util.zalo_decode(
			params, 
			self._state._config.get("secret_key")
		) 
	...
	def login(
		...
		self.onLoggedIn(
			self._state._config.get("phone_number")
		)
	...
	def disableLink(self, groupId):
		...
		response = self._post(
			'https://tt-group-wpa.chat.zalo.me/api/group/link/disable',
			data=params
		)
		...
		if data.get('error_code') == 0:
			return {
				'success': True, 
				'message':
				'Group link disabled successfully.'
			}
		else:
			error_code = data.get('error_code')
			error_message = data.get('error_message', 'Unknown error from API.')
			raise ZaloAPIException(
	...
	def newLink(self, groupId):
		...
		response = self._post(
			'https://tt-group-wpa.chat.zalo.me/api/group/link/new', 
			data=params
		)
		try:
			data = response.json()
		...
		if data.get('error_code') == 0:
			...
			if new_link_info and new_link_info.get('link'):
				return {
					'success': True, 
					'new_link': new_link_info.get('link')
				}
			else:
	...
	def createGroup(
		...
		response = self._get(
			"https://tt-group-wpa.chat.zalo.me/api/group/create/v2", 
			params=params
		)
		data = response.json()
		results = data.get("data") if data.get("error_code") == 0 else None
	...
	def changeGroupSetting(self, groupId, defaultMode="default", **kwargs):
		...
		response = self._get(
			"https://tt-group-wpa.chat.zalo.me/api/group/setting/update", 
			params=params
			)
		data = response.json()
		results = data.get("data") if data.get("error_code") == 0 else None
	...
	def changeGroupOwner(self, newAdminId, groupId):
		...
		response = self._get(
			"https://tt-group-wpa.chat.zalo.me/api/group/change-owner", 
			params=params
		)
		data = response.json()
		results = data.get("data") if data.get("error_code") == 0 else None
	...
	def blockUsersInGroup(self, members, groupId):
		...
		response = self._get(
			"https://tt-group-wpa.chat.zalo.me/api/group/blockedmems/add", 
			params=params
		)
		data = response.json()
		results = data.get("data") if data.get("error_code") == 0 else None
	...
	def addGroupAdmins(self, members, groupId):
		...
		response = self._get(
			"https://tt-group-wpa.chat.zalo.me/api/group/admins/add", 
			params=params
		)
		data = response.json()
		results = data.get("data") if data.get("error_code") == 0 else None
	...
	def removeGroupAdmins(self, members, groupId):
		...
		response = self._get(
			"https://tt-group-wpa.chat.zalo.me/api/group/admins/remove", 
			params=params
		)
		data = response.json()
		results = data.get("data") if data.get("error_code") == 0 else None
	...
	def unpinGroupMsg(self, pinId, pinTime, groupId):
		...
		response = self._get(
			"https://groupboard-wpa.chat.zalo.me/api/board/unpinv2", 
			params=params
		)
		data = response.json()
		results = data.get("data") if data.get("error_code") == 0 else None
	...
	def viewPollDetail(self, pollId):
		...
		response = self._get(
			"https://tt-group-wpa.chat.zalo.me/api/poll/detail",
			params=params
		)
		data = response.json()
		results = data.get("data") if data.get("error_code") == 0 else None
	...
	def send(
		...
		if message.mention:
			return self.sendMentionMessage(
				message, 
				thread_id, 
				ttl
			)
